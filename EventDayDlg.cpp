...
// EventDayDlg.cpp : implementation file
//
...
CEventDayDlg::CEventDayDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CEventDayDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CEventDayDlg)
	m_tTime = 0;
	m_sComment = _T("");
	m_nAmount = 0;
	m_bCritical = FALSE;
...
void CEventDayDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CEventDayDlg)
	...
	DDV_MinMaxInt(pDX, m_nAmount, 1, 30);
...
BEGIN_MESSAGE_MAP(CEventDayDlg, CDialog)
	//{{AFX_MSG_MAP(CEventDayDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CEventDayDlg message handlers

void CEventDayDlg::OnAdd() 
{
	CProfileName dlg;
...
void CEventDayDlg::OnSelchangeList() 
{
	EnableButtons();
}

BOOL CEventDayDlg::OnInitDialog() 
{
	CDialog::OnInitDialog();
	m_ctlSpin.SetRange(1, 30);
...
void CEventDayDlg::EnableButtons()
{
...
void CEventDayDlg::Fill(RconEventDay* pEvent)
{
	CString s;
	pEvent->m_nAmount = m_nAmount;
	pEvent->m_tTime = m_tTime.GetTime();
...
void CEventDayDlg::Empty(RconEventDay* pEvent)
{
	m_nAmount = pEvent->m_nAmount;
	m_tTime = pEvent->m_tTime;
